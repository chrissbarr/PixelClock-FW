...
#define __LIB8STATIC static inline
/// Define a LIB8TION member function as always static inline
#define __LIB8STATIC_ALWAYS_INLINE static inline
...
__LIB8STATIC float sfract15ToFloat( sfract15 y)
{
    return y / 32768.0;
}
...
__LIB8STATIC sfract15 floatToSfract15( float f)
{
    return f * 32768.0;
}
...
/// with 8-bit fraction
__LIB8STATIC uint8_t lerp8by8( uint8_t a, uint8_t b, fract8 frac)
{
    uint8_t result;
    if( b > a) {
        uint8_t delta = b - a;
...
__LIB8STATIC uint16_t lerp16by16( uint16_t a, uint16_t b, fract16 frac)
{
    uint16_t result;
    if( b > a ) {
        uint16_t delta = b - a;
...
/// Linear interpolation between two unsigned 16-bit values,
/// with 8-bit fraction
__LIB8STATIC uint16_t lerp16by8( uint16_t a, uint16_t b, fract8 frac)
{
    uint16_t result;
    if( b > a) {
        uint16_t delta = b - a;
...
/// with 8-bit fraction
__LIB8STATIC int16_t lerp15by8( int16_t a, int16_t b, fract8 frac)
{
    int16_t result;
    if( b > a) {
        uint16_t delta = b - a;
...
/// Linear interpolation between two signed 15-bit values,
/// with 8-bit fraction
__LIB8STATIC int16_t lerp15by16( int16_t a, int16_t b, fract16 frac)
{
    int16_t result;
    if( b > a) {
        uint16_t delta = b - a;
...
__LIB8STATIC uint8_t map8( uint8_t in, uint8_t rangeStart, uint8_t rangeEnd)
{
...
__LIB8STATIC uint8_t ease8InOutQuad( uint8_t i)
{
    uint8_t j = i;
    if( j & 0x80 ) {
        j = 255 - j;
...
// smaller and faster.
__LIB8STATIC uint8_t ease8InOutQuad(uint8_t val) {
    uint8_t j=val;
    asm volatile (
      "sbrc %[val], 7 \n"
      "com %[j]       \n"
...
__LIB8STATIC uint16_t ease16InOutQuad( uint16_t i)
{
    uint16_t j = i;
    if( j & 0x8000 ) {
        j = 65535 - j;
...
__LIB8STATIC fract8 ease8InOutCubic( fract8 i)
{
...
#if (EASE8_C == 1) || defined(FASTLED_DOXYGEN)
__LIB8STATIC fract8 ease8InOutApprox( fract8 i)
{
    if( i < 64) {
...
#elif EASE8_AVRASM == 1
__LIB8STATIC uint8_t ease8InOutApprox( fract8 i)
{
...
///
__LIB8STATIC uint8_t triwave8(uint8_t in)
{
    if( in & 0x80) {
        in = 255 - in;
    }
...
__LIB8STATIC uint8_t quadwave8(uint8_t in)
{
...
__LIB8STATIC uint8_t cubicwave8(uint8_t in)
{
...
__LIB8STATIC uint8_t squarewave8( uint8_t in, uint8_t pulsewidth=128)
{
...
__LIB8STATIC uint16_t beat88( accum88 beats_per_minute_88, uint32_t timebase = 0)
{
...
/// @param timebase the time offset of the wave from the millis() timer
__LIB8STATIC uint16_t beat16( accum88 beats_per_minute, uint32_t timebase = 0)
{
...
/// @param beats_per_minute the frequency of the wave, in decimal
/// @param timebase the time offset of the wave from the millis() timer
__LIB8STATIC uint8_t beat8( accum88 beats_per_minute, uint32_t timebase = 0)
{
...
__LIB8STATIC uint16_t beatsin88( accum88 beats_per_minute_88, uint16_t lowest = 0, uint16_t highest = 65535,
...
/// @param timebase the time offset of the wave from the millis() timer
/// @param phase_offset phase offset of the wave from the current position
__LIB8STATIC uint16_t beatsin16( accum88 beats_per_minute, uint16_t lowest = 0, uint16_t highest = 65535,
                               uint32_t timebase = 0, uint16_t phase_offset = 0)
{
...
/// @param timebase the time offset of the wave from the millis() timer
/// @param phase_offset phase offset of the wave from the current position
__LIB8STATIC uint8_t beatsin8( accum88 beats_per_minute, uint8_t lowest = 0, uint8_t highest = 255,
...
__LIB8STATIC uint16_t seconds16()
{
...
/// "every N time-periods" mechanism
__LIB8STATIC uint16_t minutes16()
{
    uint32_t ms = GET_MILLIS();
    uint16_t m16;
...
__LIB8STATIC uint16_t div1024_32_16( uint32_t in32)
{
    uint16_t out16;
#if defined(__AVR__)
    asm volatile (
...
/// second long.
__LIB8STATIC uint16_t bseconds16()
{
    uint32_t ms = GET_MILLIS();
    uint16_t s16;
...
// e.g., __LIB8STATIC seconds16, and we'd be able to use these
