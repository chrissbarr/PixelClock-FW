#ifndef __INC_PLIB8TION_RANDOM_H
#define __INC_PLIB8TION_RANDOM_H
...
__LIB8STATIC uint16_t random16()
{
    rand16seed = APPLY_FASTLED_RAND16_2053(rand16seed) + FASTLED_RAND16_13849;
    return rand16seed;
}
...
__LIB8STATIC uint8_t random8(uint8_t lim)
{
    uint8_t r = random8();
    r = (r*lim) >> 8;
    return r;
...
__LIB8STATIC uint8_t random8(uint8_t min, uint8_t lim)
{
...
/// @param lim the upper bound for the result, exclusive
__LIB8STATIC uint16_t random16( uint16_t lim)
{
    uint16_t r = random16();
...
/// @param min the lower bound for the random number, inclusive
/// @param lim the upper bound for the random number, exclusive
__LIB8STATIC uint16_t random16( uint16_t min, uint16_t lim)
{
...
__LIB8STATIC void random16_set_seed( uint16_t seed)
{
    rand16seed = seed;
}

/// Get the current seed value for the random number generator
__LIB8STATIC uint16_t random16_get_seed()
{
    return rand16seed;
}

/// Add entropy into the random number generator
__LIB8STATIC void random16_add_entropy( uint16_t entropy)
{
    rand16seed += entropy;
}
